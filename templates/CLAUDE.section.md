...
章立ては **分類（機能 > 画面）→ 観点 → ページ**。分類は `details({ 分類: ['機能', '画面'] })` で宣言する
（2 層まで）。書かなければ describe の入れ子がそのまま章立てになる。
...
- `details({分類})` — 報告書の章立て（機能 > 画面）。2 層まで。書かなければ describe の入れ子を使う。
...
- **章立ては `分類`（機能 > 画面）で決める。** 2 層まで。同じ画面の話を同じ名前で書けば 1 か所にまとまる。
  揺れる（「ファイル共有」「共有」）と章が割れるので、プロジェクトの中で名前を揃える。
