...
- 報告書は観点ごとに別ページ。表紙から目的の観点へ飛べる（`--single` で 1 ファイルにもできる）
...
報告書は **観点ごとに別ページ**。表紙（`report.html`）に概要・要確認・前提・観点の一覧・マトリクスが載り、
観点名やマトリクスの記号を押すと、その観点のページ（マトリクス 1 枚とケースの詳細）へ移る。
1 ファイルにまとめたいときは `report --single`。
...
└── 2026-08-31_212000/            実行ごと。上書きしない
    ├── report.html               報告書の表紙（概要・要確認・前提・観点一覧・マトリクス）
    ├── pages/                     観点ごとのページ（マトリクス 1 枚＋ケースの詳細、画像埋め込み）
...
| コマンド | すること |
|---|---|
| `report [<実行ID>]` | 報告書を作る。表紙 + 観点ごとのページ（`--single` で 1 ファイル、`--no-images` で軽く） |
...
| したいこと | やりかた |
|---|---|
| 1 人に渡す | `report --single` で 1 ファイルにして添付（画像込み。印刷で PDF） |
